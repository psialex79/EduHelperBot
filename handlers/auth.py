...
from db_handlers.auth_handlers import is_registered_teacher, is_registered_student, add_to_waiting_list
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    if is_registered_teacher(user_id):
        ...
        if latest_assignment:
            ...
            if latest_assignment.get('is_photo', False):
                await message.answer_photo(photo=file_id, caption=text_messages.LATEST_ASSIGNMENT_PHOTO_CAPTION)            
            else:
                await message.answer_document(document=file_id, caption=text_messages.LATEST_ASSIGNMENT_DOC_CAPTION)        
